...
export const commonValidators = {
  ...
  category: (validCategories: string[]) =>
    body('category')
...
export const pluginValidators = {
  create: [
    ...
    body('supportedDevices').optional().isArray().withMessage('Supported devices must be an array'),
  ],
  ...
  update: [
    ...
    body('pricingType').optional().isIn(['free', 'freemium', 'paid', 'subscription']),
    body('price').optional().isFloat({ min: 0, max: 9999.99 }),
    commonValidators.tags,
    commonValidators.url('homepageUrl'),
    commonValidators.url('repositoryUrl'),
    commonValidators.url('documentationUrl'),
  ...
  search: [
    ...
    query('pricingType').optional().isIn(['free', 'freemium', 'paid', 'subscription']),
    query('minRating')
      .optional()
      .isFloat({ min: 0, max: 5 })
  ...
  install: [
    ...
    body('version')
      ...
      .matches(/^\d+\.\d+\.\d+(-[a-zA-Z0-9.-]+)?$/)
      .withMessage('Version must follow semantic versioning'),
    body('autoUpdate').optional().isBoolean().withMessage('Auto update must be a boolean'),
  ],
};
...
export const reviewValidators = {
  ...
  update: [
    ...
    body('comment').optional().isLength({ max: 2000 }).trim(),
  ],
...
export const orderValidators = {
  create: [
    ...
    body('items.*.pluginId').isUUID().withMessage('Each item must have a valid plugin ID'),
    body('items.*.quantity')
      .optional()
      .isInt({ min: 1, max: 100 })
...
export const userValidators = {
  ...
  updateRole: [
    commonValidators.uuid,
    body('role').isIn(['user', 'developer', 'moderator', 'admin']).withMessage('Invalid user role'),
  ],
