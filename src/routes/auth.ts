...
router.get('/permissions',
  ...
  async (req, res) => {
    try {
      ...
      const permissions = commonResources.reduce((acc, resource) => {
        acc[resource] = commonActions.reduce((resourcePerms, action) => {
      ...
      res.json({
        ...
        data: {
          role: req.user!.role,
          permissions,
          canAccess: commonResources.reduce((acc, resource) => {
            acc[resource] = req.permissions!.canAccess(resource);
            return acc;
          }, {} as Record<string, boolean>),
          canModify: commonResources.reduce((acc, resource) => {
