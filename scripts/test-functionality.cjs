...
const tests = {
  ...
  environmentConfig: {
    ...
    test: () => {
      ...
      const requiredVars = ['VITE_CURLEC_PUBLIC_KEY', 'VITE_ENCRYPTION_KEY', 'VITE_JWT_SECRET'];
  ...
  routes: {
    ...
    test: () => {
      ...
      const routes = ['/cart', '/payment/success', '/payment/cancel', '/orders/:orderId'];
  ...
  testFramework: {
    ...
    test: () => {
      ...
      if (testFiles.length < 3) {
        return {
          success: false,
          message: `Only ${testFiles.length} test files found, expected at least 3`,
        };
      }
